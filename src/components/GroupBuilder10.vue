    ...
    methods: {
      ...
      drop(e) {
        ...
        if (this.sourceContainerClassName === 'ability' && currentGroupId === this.groupId) {
          //grab the object in the drop collection and compare the slotTypes
          //the originSlot is the object that is being dragged from.
          const originSlot = collection[this.oldIndex];
          const targetSlot = collection[dropIndex];
          //if the slot types aren't equivalent to each other exit out;
          if (targetSlot.slotType !== originSlot.slotType) return;
          //other wise continue with the alterations.
          collection.splice(dropIndex, 1, collection.splice(this.oldIndex, 1, collection[dropIndex])[0]);
        } else {
          const targetSlot = collection[dropIndex];
          if (this.dragData.slotType !== targetSlot.slotType || targetSlot.locked) return;
          targetSlot.data = this.dragData;
        }
