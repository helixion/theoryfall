<template>
    <div class="outer-box">
        <div class="inner-box" @drop="_onDrop" @dragover.prevent>
            ...
            <div :class="['ability', { selected: selectedNodeIndex === index}]" v-for="(ability, index) in tray" :key="index" :data-id="index">
                <div v-if="ability" class="icon" draggable="true" @touchstart="selectNode(index)" @dragstart="_onDragStart($event, index)" @dragend="_onDragEnd($event,index)" @mousedown.right.prevent="_remove($event, index)">
...
.abilities {
    ...
    .ability {
        ...
        margin: 0 10px;
        &.selected {
            border: 1px solid rgb(119, 221, 119);
        }
        .icon {
            display: flex;
            display: -webkit-flex;
