...
export default {
  ...
  methods: {
     createPartyMember() {
        ...
        const arr = this.generateCollection(slot, 'active', 10)
            .concat(this.generateCollection(slot, 'basic', 3))
            .concat(this.generateCollection(slot, 'passive', 5));
     ...
     generateCollection(collection, type, n) {
         let arr = [];
         for (let i = 0; i <= n, i++;) {
             Object.assign(collection, { slotType: type });
             //target the last two elements 
             if (type !== 'basic') {
                 if (i === n || i === n-1) {
